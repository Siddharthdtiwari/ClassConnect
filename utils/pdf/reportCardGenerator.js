...
async function drawStudentReport(doc, student, stats) {
  ...
  if (stats.recentScores && stats.recentScores.length > 0) {
    stats.recentScores.forEach((score, i) => {
      ...
      doc.fillColor(score.score == null ? '#dc2626' : (score.percentage >= 80 ? '#059669' : score.percentage >= 50 ? '#d97706' : '#dc2626')).font('Times-Bold').text(score.score != null && score.percentage != null ? score.percentage + '%' : 'ABS', curX, cursorY + 8, { width: academicWidths[5] });
      cursorY += 25;
    });
  } else {
